...
def print_router_info(router_data):
    ...
    for entry in router_data:
        x.add_row([entry['tenant_name'], entry['router_name'], entry['router_status']])
    return x
...
def asr_router_vrf_info(router_data):
    ...
    x.padding_width = 2
    for entry in router_data:
        x.add_row([entry['tenant_name'], entry['router_name'], entry['router_detail']['vrfname'], ', '.join(entry['router_detail']['interfaces']), entry['router_detail']['status']])
    return x
...
def asr_ipnat_pool_info(router_data):
    ...
    x.padding_width = 2
    for entry in router_data:
        x.add_row([entry['tenant_name'], entry['router_name'], entry['ipnatpool_data']['vrfname'], 
                  entry['ipnatpool_data']['nat_pool_name'], entry['ipnatpool_data']['start_ip'], 
                  entry['ipnatpool_data']['end_ip'], entry['ipnatpool_data']['netmask'], 
...
def asr_iproute_info(router_data):
    ...
    x.padding_width = 2
    for entry in router_data:
        x.add_row([entry['tenant_name'], entry['router_name'], entry['iproute_data']['vrfname'],
                  entry['iproute_data']['prefix'], entry['iproute_data']['mask'],
                  entry['iproute_data']['interface'],
                  entry['iproute_data']['next_hop_address'], entry['iproute_data']['status']])
    return x
...
def asr_network_vrf_info(router_data):
    ...
    for entry in router_data:
        for interface in entry['interface_data']:
            y.add_row([entry['tenant_name'], entry['router_name'], entry['network_vlan'][interface['vlan_id']], 
...
def main():
    ...
    for tenant in tenant_data:
        ...
        for i in range(1, NETWORK_COUNT+1):
            ...
            test_data.append(create_network(tenant, router, network_index, network_cidr))
        ...
        if ENABLE_ASR_VERIFICATION:
            ...
            asr_verify_cmd = GetASRCmd(asr_host=ASR_HOST,
                                             ...
                                             asr_slots=["0"])
            router_detail = {'vrfname': vrfname, 'interfaces': '', 'status': ''}
            ipnatpool_data = {'vrfname': vrfname, 'nat_pool_name': '', 'start_ip': '', 'end_ip': '', 'netmask': '', 'status': ''}
            iproute_data = {'vrfname': vrfname, 'prefix': '', 'mask': '', 'interface': '', 'next_hop_address': '', 'status': ''}
            interface_data = []
            nat_data = []
            try:
                ...
                print "\n"
                print "[ERROR] Caught exception on ASR Verification : %s" % (exc.message)
                print "\n"
                asr_report = False
